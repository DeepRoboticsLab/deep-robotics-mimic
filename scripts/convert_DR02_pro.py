...
# Robot model joint order (matches MuJoCo XML & URDF traversal)
...
def run_fk_for_file(
    ...
    print(f"    -> {num_joints} joints, {num_bodies} bodies")
...
def main():
    """Set up simulation and run FK conversion for a single file."""
    ...
    print(f"[INFO] Creating scene with {num_envs} envs (frames: {max_frames})")
    ...
    run_fk_for_file(sim, scene, input_path, output_path, output_fps)

    elapsed = time.time() - t_start
    print(f"\n[DONE] Processed 1 file in {elapsed:.1f}s")
    sys.exit(0)
