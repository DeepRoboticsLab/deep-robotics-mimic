...
def export_motion_policy_as_onnx(
    ...
    """Export the trained policy and motion data as an ONNX model.

    The exported model takes (obs, time_step) as inputs and outputs
    actions plus precomputed motion data (joint pos/vel, body poses/vels).
    """
...
class _OnnxMotionPolicyExporter(_OnnxPolicyExporter):
    """ONNX exporter that embeds motion data alongside the policy network."""

    def __init__(self, env: ManagerBasedRLEnv, actor_critic, normalizer=None, verbose=False):
        super().__init__(actor_critic, normalizer, verbose)
        # --- Load motion data from the environment's motion command ---
    ...
    def forward(self, x, time_step):
        """Forward pass: run policy on obs and return motion data for the given time step."""
    ...
    def export(self, path, filename):
        """Export the model to ONNX format with dual inputs (obs, time_step)."""
        self.to("cpu")
...
def attach_onnx_metadata(env: ManagerBasedRLEnv, run_path: str, path: str, filename="policy.onnx") -> None:
    """Attach robot metadata (joint names, PD gains, body names, etc.) to the ONNX model."""
